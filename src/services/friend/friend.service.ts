...
export const createFriendRequest = async (data: { receiver: string }): Promise<ResponsePagination<FriendType>> => {
  const result = await post(import.meta.env.VITE_APP_API + 'friends', data);
  return result as ResponsePagination<FriendType>;
};
export const acceptFriendRequest = async (data: { friendId: string }): Promise<ResponsePagination<FriendType>> => {
